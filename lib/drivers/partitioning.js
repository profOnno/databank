...
var PartitioningDatabank = function(params) {
    ...
    var bank = this,
        ...
        getBank = function(type, callback) {
            var tb;
            if (banks[type]) {
                if (typeConnected.connected[type]) {
                    callback(null, banks[type]);
                } else {
                    typeConnected.on("connect:"+type, function() {
                        callback(null, banks[type]);
                    });
                    typeConnected.on("error:"+type, function(err) {
                        callback(err, null);
                    });
                }
            } else if (params[type]) {
                ...
                banks[type] = Databank.get(params[type].driver, params[type].params);
                if (!typeConnected.connected) {
                    typeConnected.connected = {};
                }
                typeConnected.connected[type] = false;
                banks[type].connect(params[type].params, function(err) {
                    if (err) {
                        typeConnected.emit("error:"+type, err);
                        callback(err, null);
                    } else {
                        typeConnected.connected[type] = true;
                        setImmediate(function() {
                            callback(null, banks[type]);
                        });
                        typeConnected.emit("connect:"+type);
                    }
                });
            } else {
