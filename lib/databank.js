...
Databank.get = function(driver, params) {
    ...
    var core = ['memory', 'caching'],
        canon = driver.toLowerCase(),
        databank = module.exports,
        factory,
        cls;

    if (core.indexOf(canon) !== -1) {
        factory = require(Databank.localDriver(driver));
    } else if (Databank.drivers.hasOwnProperty(driver)) {
        factory = Databank.drivers[driver];
    } else {
        factory = require("databank-" + canon);
    }

    cls = factory(databank);
